...
namespace Enyim.Caching
{
	...
	public partial class MemcachedClient : IMemcachedClient, IMemcachedResultsClient
	{
...
#if NETFX
		internal static readonly IMemcachedClientConfiguration DefaultSettings = System.Configuration.ConfigurationManager.GetSection("enyim.com/memcached") as MemcachedClientSection;
		...
		{
			MemcachedClientSection section = (MemcachedClientSection)System.Configuration.ConfigurationManager.GetSection(sectionName);
			if (section == null)
				throw new System.Configuration.ConfigurationErrorsException("Section " + sectionName + " is not found.");
...
#if NETSTANDARD
			var transcoder = configuration.CreateTranscoder();
			if (transcoder == null) throw new ArgumentException("Configuration must return a non-null Transcoder");
#else
			var transcoder = configuration.CreateTranscoder() ?? new DefaultTranscoder();
#endif
		...
		public MemcachedClient(IServerPool pool, IMemcachedKeyTransformer keyTransformer, ITranscoder transcoder, IPerformanceMonitor performanceMonitor = null)
		{

...
namespace DemoApp
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			Console.WriteLine(nsc.Store(StoreMode.Set, "1", "1"));
			Console.WriteLine(nsc.Get("1"));
			...
			var emc = new MemcachedClientConfiguration();
			emc.Protocol = MemcachedProtocol.Binary;
			emc.AddServer("192.168.2.200:11211");
			emc.AddServer("192.168.2.202:11211");
			var mc = new MemcachedClient(emc);
			...
			while (true)
			{
				var res = mc.Store(StoreMode.Set, i.ToString(), i);
				var mod = res ? 1000 : 1000000;

				if (i % mod == 0 || res != last)
				{
					Console.ForegroundColor = res ? ConsoleColor.White : ConsoleColor.Red;
					Console.Write(".");
					last = res;
				}

...
namespace Enyim.Caching
{
	...
	public class MemcachedClient2 : IDisposable
	{
		...
		{
			var hashedKey = this.pool.KeyTransformer.Transform(key);
			var node = this.pool.NodeLocator.Locate(hashedKey);

			if (node != null && node.IsAlive)
			{
				var command = this.opFactory.Get(hashedKey);
				if (node.Execute(command))
				{
					value = this.pool.Transcoder.Deserialize(command.Result);
		...
		{
			return this.Store(mode, key, value, MemcachedClient2.GetExpiration(validFor, null));
		}
		...
		{
			return this.Store(mode, key, value, MemcachedClient2.GetExpiration(null, expiresAt));
		}
		...
		private bool Store(StoreMode mode, string key, object value, uint expires)
		{
			...
			if (node != null && node.IsAlive)
			{
				var item = this.pool.Transcoder.Serialize(value);
				var command = this.opFactory.Store(mode, hashedKey, item, expires);
		...
		private ulong Mutate(MutationMode mode, string key, ulong defaultValue, ulong delta, uint expires)
		{
			...
			if (node != null && node.IsAlive)
			{
				var command = this.opFactory.Mutate(mode, hashedKey, defaultValue, delta, expires);
		...
		{
			return this.Mutate(MutationMode.Increment, key, defaultValue, delta, MemcachedClient2.GetExpiration(validFor, null));
		}
		...
		{
			return this.Mutate(MutationMode.Increment, key, defaultValue, delta, MemcachedClient2.GetExpiration(null, expiresAt));
		}
		...
		{
			return this.Mutate(MutationMode.Decrement, key, defaultValue, delta, MemcachedClient2.GetExpiration(validFor, null));
		}
		...
		{
			return this.Mutate(MutationMode.Decrement, key, defaultValue, delta, MemcachedClient2.GetExpiration(null, expiresAt));
		}
		...
		public bool Concatenate(ConcatenationMode mode, string key, ArraySegment<byte> data)
		{
			...
			if (node != null && node.IsAlive)
			{
				var command = this.opFactory.Concat(mode, hashedKey, data);
		...
		public void FlushAll()
		{
			...
			{
				var command = this.opFactory.Flush();
		...
		public bool Remove(string key)
		{
			...
			if (node != null && node.IsAlive)
			{
				var command = this.opFactory.Delete(hashedKey);
